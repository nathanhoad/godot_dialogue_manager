...
#### Extra Game States vs Locals

Extra game states and locals are similar in that they only exist during the current conversation, but there are some differences:

**Extra game states:**
- Passed as a parameter when starting dialogue
- Accessed directly by their property/key names (e.g., `level`, `health`)
- Can include objects with methods that can be called from dialogue

**Locals:**
- Created within dialogue using `set locals.variable_name = value`
- Must be prefixed with `locals.` (e.g., `locals.counter`)
- Simple variables only, no methods

For example, if you pass `{"level": 10, "health": 100}` as extra game state:
...
You can reference these directly in dialogue:
...
You **cannot** access extra game states using `locals.level` or modify them with `set locals.level = 5`. Similarly, you **cannot** access variables created with `set locals.counter = 0` by just using `counter`, you must use the `locals.` prefix.

#### Advanced Example: Number Guessing Game

Here is a more complex example using locals for game logic:
