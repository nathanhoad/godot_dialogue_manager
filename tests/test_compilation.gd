...
func test_get_line_type() -> void:
	assert(compilation.get_line_type("~ some_cue") == DMConstants.TYPE_CUE, "Cue should be cue.")

	assert(compilation.get_line_type("=> cue") == DMConstants.TYPE_GOTO, "Goto should be goto.")
	assert(compilation.get_line_type("=>< cue") == DMConstants.TYPE_GOTO, "Goto should be goto.")
	...
	assert(compilation.get_line_type("- prompt [if condition /] => cue") == DMConstants.TYPE_RESPONSE, "Response should be response.")
	assert(compilation.get_line_type("- prompt => cue") == DMConstants.TYPE_RESPONSE, "Response should be response.")
	...
	assert(compilation.get_line_type("Character: Dialogue => cue") == DMConstants.TYPE_DIALOGUE, "Dialogue should be dialogue.")
	assert(compilation.get_line_type("% Character: Dialogue") == DMConstants.TYPE_DIALOGUE, "Dialogue should be dialogue.")
